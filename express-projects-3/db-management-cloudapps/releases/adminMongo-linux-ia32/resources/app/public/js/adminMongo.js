$(document).ready(function(){
    ...
    $(document).on('click', '#coll_create', function(){
        if($('#new_coll_name').val() !== '') {
            $.ajax({
                type: 'POST',
                url: $('#app_context').val() + '/v1/' + $('#db_name').val() + '/admin/coll_create/',
                headers: {
                    'Content-type': 'application/json',
                    Authorization: 'Bearer xvvmdhasler-cloudffd-apps4345-xcxcx-5654vbvh-56546bvgvbnbn-dsjdsh7834343-querybasedfdjfdjv-434hmvnxcmnfjksdf'
                },
                data: JSON.stringify({'modelName': $('#new_coll_name').val(), "schema":{"Id":{"type":"string","unique":true},"name":{"type":"string","unique":true}}})
            })
                .done(function (data) {
                    $('#del_coll_name').append('<option>' + $('#new_coll_name').val() + '</option>');
                    $('#new_coll_name').val('');
                    show_notification($('#new_coll_name').val()+' collection created successfully', 'success', true);
                })
                .fail(function (data) {
                    show_notification('Failed to create '+$('#new_coll_name').val()+' collection', 'danger');
                });
    ...
    $(document).on('click', '#coll_delete', function(){
        if(confirm('WARNING: Are you sure you want to delete this collection and all documents?') === true){
            $.ajax({
                method: 'DELETE',
                url: $('#app_context').val() + '/v1/' + $('#db_name').val() + '/admin/coll_delete/',
                headers: {
                    'Content-type': 'application/json',
                    Authorization: 'Bearer xvvmdhasler-cloudffd-apps4345-xcxcx-5654vbvh-56546bvgvbnbn-dsjdsh7834343-querybasedfdjfdjv-434hmvnxcmnfjksdf'
                },
                data: JSON.stringify({'modelName': $('#del_coll_name option:selected').text(),'conn':$('#conn_name').val()})
            })
            .done(function(data){
        ...
        if ($('#new_db_name').val() !== '') {
            $.ajax({
                ...
                .done(function (data) {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: $('#app_context').val() + '/v1/' + $('#new_db_name').val() + '/users/',
                        headers: {
                            'Content-type': 'application/json',
                            Authorization: 'Bearer xvvmdhasler-cloudffd-apps4345-xcxcx-5654vbvh-56546bvgvbnbn-dsjdsh7834343-querybasedfdjfdjv-434hmvnxcmnfjksdf'
                        }
    ...
    $(document).on('click', '#user_create', function(){
        ...
        $.ajax({
            ...
            data: {
                ...
                'roles': $('#new_user_roles option:selected').val()
            }
        })
        .done(function(data){
        ...
        // clear items
            ...
            $('#new_user_roles').val($('#new_user_roles option:first').val());
        })
        .fail(function(data){
            show_notification(data.responseJSON.msg, 'danger');
        });
    ...
    $(document).on('click', '#user_delete', function(){
        if(confirm('WARNING: Are you sure you want to delete '+$('#del_user_name option:selected').text()+' user?') === true){
                $.ajax({
                method: 'POST',
...
function show_notification(msg, type, reload_page, timeout){
    ...
    if(reload_page === true){
        $('.btnConnConnect').attr('disabled',true);
    }
    ...
    $('#notify_message').slideDown(600).delay(timeout).slideUp(600, function(){
        if(reload_page === true){
            $('.btnConnConnect').attr('disabled',false);
            location.reload();
        }
    });
}

 //update permission by @sourabh
function checkForValid(coll_name ,permission_id, type,value) {

    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: $('#app_context').val() + '/v1/' + $('#db_name').val() + '/admin/role_permissions/' + permission_id,
        headers: {
            'Content-type': 'application/json',
            Authorization: 'Bearer xvvmdhasler-cloudffd-apps4345-xcxcx-5654vbvh-56546bvgvbnbn-dsjdsh7834343-querybasedfdjfdjv-434hmvnxcmnfjksdf'
        }
        ...
        .done(function (response) {
            response.Result.permission[coll_name][type] = value;
            $.ajax({
                type: 'PUT',
                dataType: 'json',
                url: $('#app_context').val() + '/v1/' + $('#db_name').val() + '/admin/role_permissions/',
                headers: {
                    'Content-type': 'application/json',
                    Authorization: 'Bearer xvvmdhasler-cloudffd-apps4345-xcxcx-5654vbvh-56546bvgvbnbn-dsjdsh7834343-querybasedfdjfdjv-434hmvnxcmnfjksdf'
                },
                ...
                .fail(function (data) {
                    show_notification('Failed to update the Permission object', 'danger');
                });
        })
        .fail(function (data) {
            show_notification('Failed to get the Permission object', 'danger');
        });
}
