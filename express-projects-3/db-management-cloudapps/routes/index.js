...
var connStr = 'mongodb://superAdmin1:Qwerty12345!@127.0.0.1:61725/cloud-backend-service?authSource=admin';
var db_connection = mongoose.connect(connStr, { useMongoClient: true });

// save user to database
var User = db_connection.model('User', User.UserSchema);

var test = new User({
    'username': 'resul@brixxs.com',
    'password': 'Qwerty@123!'
});
User.findOne({ username: 'resul@brixxs.com' }, function (err, res) {
    //console.log(err);
    if (!res) {
        test.save();
    }
...
router.post('/app/login_action', function (req, res, next) {
    User.getAuthenticated(req.body.inputUser, req.body.inputPassword, function (err, user, reason) {
        if (err) throw err;
        if (user) {
        ...
        switch (reason) {
            ...
            case reasons.MAX_ATTEMPTS:
                ...
                try {
                    EmailService.sendEmail(req.body.inputUser, 'oauth', getClientIp(req)); // send mail
                } catch (e) {
                    console.log("Got Error while sending mail:");
                }
                res.render('login', {
                    message: 'Your account is locked, Please contact your administrator for unlocked',
                    helpers: req.handlebars.helpers
                });
                break;
        }
    ...
    common.get_db_status(mongo_db, function (err, db_status) {
        common.get_db_list(uri, mongo_db, function (err, db_list) {
            if (db_list)
                ...
                if (db_list[i] == 'cloud-backend-service') {
                    db_list.splice(i, 1);
                    break;
                }
            }
...
// manage roles api
router.get('/app/:conn/:db/roles', function (req, res, next) {
    var connection_list = req.app.locals.dbConnections;
    var uri = {};
    uri.database = null;
    // Check for existance of connection
    ...
    common.get_db_stats(mongo_db, req.params.db, function (err, db_stats) {
       ...
       common.get_db_list(uri, mongo_db, function (err, db_list) { // added by anil
            mongo_db.command({ usersInfo: 1 }, function (err, conn_users) {
                mongo_db.listCollections().toArray(function (err, collection_list) {

                    mongo_db.collection('role_permissions').find({}).toArray(function (err, permissions) {
                        mongo_db.collection('roles').find({}).toArray(function (err, roles) {
                            for (var i in permissions) {
                                for (var x in roles) {
                                    if (roles[x].Id == permissions[i].RoleId) {
                                        permissions[i].name = roles[x].Name;
                                    }
                                }
                            }
                            res.render('roles', {
                                ...
                                show_db_name: true,
                                // sidebar_list: sidebar_list, Commented by anil
                                db_list: db_list, //added by Anil
